...
const RealtimeNegotiationChat = ({ isOpen, onClose, bid, rfq, userRole }) => {
  ...
  const initializePusher = () => {
    ...
    pusherRef.current = new Pusher('your-pusher-key', {
      cluster: 'us2',
      encrypted: true,
    })

    const channel = pusherRef.current.subscribe(`private-negotiation.${negotiation?.id}`)
    ...
    channel.bind('status.changed', (data) => {
      ...
      showToast(`Negotiation status changed to ${data.status_label}`, 'info')
    })
  ...
  const handleTyping = (e) => {
    ...
    if (pusherRef.current && negotiation) {
      const channel = pusherRef.current.channel(`private-negotiation.${negotiation.id}`)
...
export default RealtimeNegotiationChat
